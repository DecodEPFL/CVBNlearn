...
from benchmarks.sampling_bn import CVBNetSampled
...
def test_inference_simple(sample_size=10, uncertainty_factor=1.0, steps=100000):
    """
    Tests the inference of a continuous BN using sampling of a toy example.
    ...
    # Guess with specific uncertainty for sampling
    ...
    start_time = time()
    likelihoods = CVBNetSampled.infer(bn, guess, guess_deviation, verbose=verbose)
...
def test_inference_hard(sample_size=10, tax_dimension=1, uncertainty_factor=1.0, steps=5000000):
    """
    Tests the inference of a continuous BN using sampling of the example in examples.hard_case.
    ...
    bn.maximum_iterations = steps
    start_time = time()
    likelihood_list = CVBNetSampled.infer(bn, guess, guess_deviation, verbose=verbose)
    elapsed_time = time() - start_time
